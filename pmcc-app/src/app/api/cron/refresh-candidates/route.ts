...
function getExpiryDates() {
  ...
  return {
    long: Math.floor(longExpiry.getTime() / 1000),
    short: Math.floor(shortExpiry.getTime() / 1000),
  };
}
...
async function findPMCCOpportunity(ticker: string) {
  try {
    ...
    return {
      ...
      long_call_expiry: new Date(long * 1000).toISOString(),
      long_call_strike: leap.strike,
      long_call_price: leap.ask || leap.lastPrice,
      short_call_expiry: new Date(short * 1000).toISOString(),
